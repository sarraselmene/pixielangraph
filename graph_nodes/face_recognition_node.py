...
try:
    ...
    print(f"[WARN] Face Recognition disabled: DeepFace dependency missing ({e})")
...
try:
    ...
    print(f"[WARN] SixDRepNet not found. Head-pose filtering disabled.")
...
def run_face_recognition(state: dict) -> dict:
    ...
    if not os.path.exists(face_crops_dir):
        ...
        return {**state, "identity_map": identity_map}
